...
```
## Now i will show a brief example of my data visualization
```{r}
...
ggplot(precip_df, aes(x = lon, y = lat, fill = precipitation)) + geom_tile() +
  ...
  scale_fill_viridis_c(option = "rainbow", name = "Precipitation (mm)") +
  coord_quickmap() +
...
plot(precip_europe, main = "Precipitation - Europe in January")
maps::map('world', add = TRUE, col = "gray50")
```
...
```
```{r}
max_precip_global <- max(summary_df$max_precip, na.rm = TRUE)
min_precip_global <- min(summary_df$min_precip, na.rm = TRUE)
max_precip_global
min_precip_global
```

...
head(time)
```
## After reviewing the variables present in my file, now we can plot them to understand them
```{r}
...
```
## The purpose of the work now is to understand the mean precipitation per day in the wold in form of graphs for the 12 months in 2022
...
europe_plots
```
## There are other libraries that can work with to create cartographs , like the ones we'll use now
```{r}
library(sf)
...
```
### purpose of our study now is to analyse the total of precipitation monthly
...
monthly_total_maps <- purrr::map(monthly_totals_list[1:3], plot_monthly_total_map)
